...
import ReactFlow, { Node, Edge, Controls, Background, MiniMap, useNodesState, useEdgesState, addEdge, Connection } from "reactflow";
import "reactflow/dist/style.css";
import { AlertTriangle, CheckCircle, Loader2, ChevronRight, Info } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent } from "@/components/ui/card";
import { Sidebar } from "@/components/Sidebar";

interface KGNode { id: string; title: string; summary: string; sourceChunks: string; difficultyTier: string; sectionName?: string; }
interface KGEdge { id: string; fromNodeId: string; toNodeId: string; type: string; confidence: number; }
...
export default function GraphVerifyPage() {
  ...
  const [selected, setSelected] = useState<KGNode | KGEdge | null>(null);
  ...
  useEffect(() => {
    fetch(`/api/books/${bookId}/graph`)
      .then(r => r.json())
      .then(({ nodes: kgNodes, edges: kgEdges }) => {
        setRawNodes(kgNodes);
        const lowConf = kgEdges.some((e: KGEdge) => e.confidence < 0.6);
        setHasLowConf(lowConf);

        // Layout nodes in a DAG-ish grid
        const flowNodes: Node[] = kgNodes.map((n: KGNode, i: number) => ({
          id: n.id,
        ...
        const flowEdges: Edge[] = kgEdges.map((e: KGEdge) => ({
          id: e.id,
          source: e.fromNodeId,
          target: e.toNodeId,
          animated: e.confidence < 0.6,
          style: { stroke: e.confidence < 0.6 ? "#f97316" : "#94a3b8", strokeWidth: e.confidence < 0.6 ? 2 : 1.5 },
    ...
    fetch(`/api/books/${bookId}`).then(r => r.json()).then(({ book }) => setBookTitle(book?.title ?? ""));
  }, [bookId]);
  ...
  const handleConfirm = async () => {
    ...
    if (res.ok) router.push(`/book/${bookId}/assessment`);
      ...
      <div className="flex-1 ml-16 lg:ml-56 flex flex-col">
        ...
        <div className="flex flex-1">
          ...
          <div className="w-72 border-l border-slate-200 bg-white p-4 overflow-y-auto">
            ...
            {selected && "title" in selected && (
              <Card>
                <CardContent className="p-3 space-y-2">
                  <p className="font-semibold text-sm text-slate-900">{(selected as KGNode).title}</p>
                  <Badge variant={(selected as KGNode).difficultyTier as "default"} className="capitalize text-xs">
                    {(selected as KGNode).difficultyTier}
                  </Badge>
                  <p className="text-xs text-slate-600">{(selected as KGNode).summary}</p>
                  {(selected as KGNode).sectionName && (
                    <p className="text-xs text-slate-400">§ {(selected as KGNode).sectionName}</p>
                  )}
                  <details className="text-xs">
                    <summary className="cursor-pointer text-slate-500 flex items-center gap-1"><Info className="h-3 w-3" />Source text</summary>
                    <p className="mt-1 text-slate-600 bg-slate-50 rounded p-2 leading-relaxed">
                      {JSON.parse((selected as KGNode).sourceChunks || "[]")[0]?.slice(0, 300)}...
                    </p>
                  </details>
                </CardContent>
              </Card>
            ...
            {!selected && (
              <p className="text-xs text-slate-400 text-center mt-8">Click a node to inspect it</p>
            )}
          </div>
        </div>
      </div>
