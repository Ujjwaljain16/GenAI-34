...
import { Bell, Search, Plus, Flame, BookOpen, Clock, ChevronRight, Loader2, AlertTriangle, BrainCircuit, User, LogOut, SortAsc } from "lucide-react";
...
const STATUS_BADGE: Record<string, { label: string; color: "default" | "warning" | "success" | "info" | "secondary" }> = {
...
export default function LibraryPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/books")
      .then(r => r.json())
      .then(d => {
        setBooks((d.books ?? []).map((b: BookCard) => ({
          ...b,
          progressPct: b.totalNodes > 0 ? Math.round((b.masteredNodes / b.totalNodes) * 100) : 0,
        })));
        setLoading(false);
      });
  ...
  const globalStreak = 7; // would come from user data
  ...
  const getBookAction = (book: BookCard) => {
    if (book.status === "kg_verified") return `/book/${book.id}/verify`;
    if (book.status === "ready") return `/book/${book.id}`;
    if (book.status === "parsing" || book.status === "kg_built") return `/book/${book.id}/processing`;
    return `/book/${book.id}`;
  };
      ...
      <main className="flex-1 ml-16 lg:ml-56 p-6 max-w-5xl">
        ...
        <header className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">Welcome back, {session?.user?.name?.split(" ")[0] ?? "there"}</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            ...
            <Card className="p-4 flex items-center gap-3 bg-indigo-600 border-indigo-600">
              ...
              <div className="flex-1">
                <p className="text-sm font-semibold text-white">{estimateMinutes(totalDue)} min plan</p>
                <button onClick={() => books[0] && router.push(getBookAction(books[0]))} className="text-xs text-indigo-200 hover:text-white underline">
                  Start today's plan →
                </button>
              </div>
            </Card>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select value={sort} onChange={e => setSort(e.target.value as typeof sort)} className="text-sm border border-slate-200 rounded-lg px-2 py-1 bg-white">
        ...
        {books.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-slate-500 mb-6 max-w-sm">Upload your first book and Lexis will build a knowledge graph, assess your knowledge, then teach you concept by concept.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {/* Add Book card */}
            <button onClick={() => router.push("/upload")} className="flex flex-col items-center justify-center gap-3 p-8 rounded-xl border-2 border-dashed border-slate-200 text-slate-400 hover:border-indigo-300 hover:text-indigo-500 transition-colors min-h-[200px]">
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-slate-900 truncate text-sm">{book.title}</h3>
                        {book.author && <p className="text-xs text-slate-500 truncate">{book.author}</p>}
                      </div>
                      <Badge variant={STATUS_BADGE[book.status]?.color ?? "secondary"} className="shrink-0 text-[10px]">
                    ...
                    {book.status === "ready" && (
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <div className="text-right">
                          {book.dueToday > 0 && (
                            <p className="text-xs font-medium text-orange-600">{book.dueToday} due</p>
                          )}
                          {book.bookStreak > 0 && (
                            <p className="text-xs text-slate-500 flex items-center gap-1 justify-end">
                              <Flame className="h-3 w-3 text-orange-400" />{book.bookStreak}d
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    {(book.status === "parsing" || book.status === "kg_built") && (
                      <div className="flex items-center gap-2 mt-3 text-xs text-slate-500">
                        <Loader2 className="h-3 w-3 animate-spin" /> Building knowledge graph...
                      </div>
                    )}

                    {book.status === "kg_verified" && (
                      <div className="flex items-center gap-1 mt-3 text-xs text-amber-600">
                        <AlertTriangle className="h-3 w-3" /> Needs your review
                      </div>
                    )}
