...
import { Bell, BookOpen, Clock, Flame, Trophy, CheckCheck, ArrowLeft, Loader2 } from "lucide-react";
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/notifications").then(r => r.json()).then(({ notifications }) => {
        ...
        {notifications.length === 0 ? (
          ...
          <div className="space-y-2">
            {notifications.map(n => (
              <Card
                key={n.id}
                className={`cursor-pointer transition-all hover:shadow-sm ${!n.read ? "border-indigo-200 bg-indigo-50/50" : ""}`}
                onClick={() => { markRead(n.id); if (n.link) router.push(n.link); }}
              >
                <CardContent className="p-4 flex items-start gap-3">
                  <div className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 ${!n.read ? "bg-white shadow-sm" : "bg-slate-100"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-1">{new Date(n.createdAt).toLocaleDateString("en", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</p>
                  </div>
                  {!n.read && <div className="h-2 w-2 bg-indigo-500 rounded-full mt-1.5 shrink-0" />}
                </CardContent>
              </Card>
            ))}
          </div>
