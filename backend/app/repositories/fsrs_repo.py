...
class FsrsRepository:
    ...
    async def upsert_state(
        self, user_id: str, concept_id: str, state: FsrsState, interval_days: int
    ) -> None:
        now = datetime.now(timezone.utc)
        next_due = now + timedelta(days=interval_days)
        stmt = (
            pg_insert(ConceptFsrs.__table__)
            .values(
                user_id=user_id,
    ...
    async def log_review(
        ...
        self.session.add(
            FsrsReview(
                ...
                retrievability_before=before.retrievability,
                retrievability_after=after.retrievability,
            )
        )

    async def log_mastery_event(
        self,
        user_id: str,
        concept_id: str,
        source: str,
