...
class UserConceptState(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["concepts.id"],
            ondelete="CASCADE",
            name="user_concept_state_concept_id_fkey",
        ),
        ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            ondelete="CASCADE",
            name="user_concept_state_user_id_fkey",
        ),
        PrimaryKeyConstraint("id", name="user_concept_state_pkey"),
        UniqueConstraint(
            "user_id", "concept_id", "graph_version", name="uq_user_concept_state"
        ),
        ...
        Index(
            "idx_ucs_locked",
            "user_id",
            postgresql_where="(state = 'LOCKED'::node_state)",
        ),
...
class ConceptMastery(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "mastery_score >= 0::numeric AND mastery_score <= 1::numeric",
            name="chk_mastery_score",
        ),
        CheckConstraint(
            "mastery_state <> 'MASTERED'::mastery_state OR first_mastered_at IS NOT NULL",
            name="chk_mastery_mastered_timestamp",
        ),
        ForeignKeyConstraint(
            ["concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="concept_mastery_concept_id_fkey",
        ),
        ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            ondelete="CASCADE",
            name="concept_mastery_user_id_fkey",
        ),
        ...
        Index(
            "idx_mastery_user_concept_score", "user_id", "concept_id", "mastery_score"
        ),
