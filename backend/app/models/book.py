...
class Book(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "processing_completed_at IS NULL OR processing_started_at IS NULL OR processing_completed_at >= processing_started_at",
            name="chk_processing_time",
        ),
        CheckConstraint(
            "source_type::text = ANY (ARRAY['PDF'::character varying, 'EPUB'::character varying, 'TXT'::character varying, 'DOCX'::character varying, 'URL'::character varying]::text[])",
            name="books_source_type_check",
        ),
        ForeignKeyConstraint(
            ["owner_id"], ["users.id"], ondelete="SET NULL", name="fk_books_owner"
        ),
    ...
    )  # We'll assume the FK is to users.id
    ...
    visibility = Column(
        ENUM("PRIVATE", "PUBLIC", name="book_visibility", create_type=False),
        nullable=False,
        default="PRIVATE",
    )
    ...
    build_jobs = relationship(
        "GraphBuildJob", back_populates="book", cascade="all, delete-orphan"
    )
    graph_versions = relationship(
        "GraphVersion", back_populates="book", cascade="all, delete-orphan"
    )
...
class GraphBuildJob(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "chunks_processed IS NULL OR chunks_processed >= 0",
            name="chk_chunks_processed",
        ),
        CheckConstraint(
            "completed_at IS NULL OR started_at IS NULL OR completed_at >= started_at",
            name="chk_graph_job_time",
        ),
        CheckConstraint(
            "concepts_created IS NULL OR concepts_created >= 0",
            name="chk_concepts_created",
        ),
        CheckConstraint(
            "edges_created IS NULL OR edges_created >= 0", name="chk_edges_created"
        ),
        CheckConstraint("graph_version > 0", name="chk_graph_version_positive"),
        CheckConstraint(
            "nodes_created IS NULL OR nodes_created >= 0", name="chk_nodes_created"
        ),
        ForeignKeyConstraint(
            ["book_id"],
            ["books.id"],
            ondelete="CASCADE",
            name="graph_build_jobs_book_id_fkey",
        ),
        ForeignKeyConstraint(
            ["book_upload_id"],
            ["book_uploads.id"],
            ondelete="CASCADE",
            name="graph_build_jobs_book_upload_id_fkey",
        ),
        ...
        Index(
            "uq_completed_graph_version",
            "book_id",
            "graph_version",
            postgresql_where="(status = 'COMPLETED'::graph_build_status)",
            unique=True,
        ),
        {
    ...
    book_upload_id = Column(
        UUID(as_uuid=True), ForeignKey("book_uploads.id", ondelete="CASCADE")
    )
...
class GraphVersion(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["books.id"],
            ondelete="CASCADE",
            name="graph_versions_book_id_fkey",
        ),
        ForeignKeyConstraint(
            ["build_job_id"],
            ["graph_build_jobs.id"],
            ondelete="RESTRICT",
            name="graph_versions_build_job_id_fkey",
        ),
        ...
        Index(
            "idx_graph_versions_current",
            "book_id",
            postgresql_where="(is_current = true)",
        ),
        Index(
            "uq_current_graph_version_per_book",
            "book_id",
            postgresql_where="(is_current = true)",
            unique=True,
        ),
    ...
    build_job_id = Column(
        UUID(as_uuid=True),
        ForeignKey("graph_build_jobs.id", ondelete="RESTRICT"),
        nullable=False,
    )
...
class UserBook(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["books.id"],
            ondelete="CASCADE",
            name="user_books_book_id_fkey",
        ),
        ForeignKeyConstraint(
            ["current_chapter_id"],
            ["chapters.id"],
            ondelete="SET NULL",
            name="user_books_current_chapter_id_fkey",
        ),
        ForeignKeyConstraint(
            ["pinned_graph_version_id"],
            ["graph_versions.id"],
            ondelete="RESTRICT",
            name="user_books_pinned_graph_version_id_fkey",
        ),
        ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
    ...
    pinned_graph_version_id = Column(
        UUID(as_uuid=True),
...
class Chapter(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="chk_estimated_minutes",
        ),
        ForeignKeyConstraint(
            ["book_id"], ["books.id"], ondelete="CASCADE", name="chapters_book_id_fkey"
        ),
...
class BookUpload(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["books.id"],
            ondelete="CASCADE",
            name="book_uploads_book_id_fkey",
        ),
        ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            ondelete="CASCADE",
            name="book_uploads_user_id_fkey",
        ),
    ...
    upload_status = Column(
        ENUM("PENDING", "STORED", "FAILED", name="upload_status", create_type=False),
        nullable=False,
        default="PENDING",
    )
    created_at = Column(
...
class GraphValidationResult(Base):
    ...
    graph_version_id = Column(
        UUID(as_uuid=True), ForeignKey("graph_versions.id", ondelete="CASCADE")
    )
...
class GraphRepairLog(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["graph_versions.id"],
            ondelete="CASCADE",
            name="graph_repair_log_graph_version_id_fkey",
        ),
