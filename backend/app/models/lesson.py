...
class LessonSession(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="chk_lesson_time",
        ),
        ForeignKeyConstraint(
            ["concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="lesson_sessions_concept_id_fkey",
        ),
        ForeignKeyConstraint(
            ["curriculum_plan_id"],
            ["curriculum_plans.id"],
            ondelete="SET NULL",
            name="lesson_sessions_curriculum_plan_id_fkey",
        ),
        ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            ondelete="CASCADE",
            name="lesson_sessions_user_id_fkey",
        ),
        PrimaryKeyConstraint("id", name="lesson_sessions_pkey"),
        Index(
            "idx_active_lesson_sessions",
            "user_id",
            postgresql_where="(status = 'IN_PROGRESS'::lesson_status)",
        ),
        Index("idx_lesson_sessions_concept", "concept_id"),
        Index(
            "idx_lesson_sessions_content", "generated_content", postgresql_using="gin"
        ),
...
class TutorInteraction(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["lesson_sessions.id"],
            ondelete="CASCADE",
            name="tutor_interactions_lesson_session_id_fkey",
        ),
        ForeignKeyConstraint(
            ["question_id"],
            ["generated_questions.id"],
            ondelete="SET NULL",
            name="tutor_interactions_question_id_fkey",
        ),
        ...
        Index(
            "idx_tutor_interactions_question",
            "question_id",
            postgresql_where="(question_id IS NOT NULL)",
        ),
