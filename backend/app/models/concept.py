...
class Concept(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="chk_concept_difficulty",
        ),
        CheckConstraint(
            "estimated_minutes IS NULL OR estimated_minutes > 0",
            name="chk_concept_minutes",
        ),
        CheckConstraint("graph_version > 0", name="chk_concept_graph_version"),
        ForeignKeyConstraint(
            ["book_id"], ["books.id"], ondelete="CASCADE", name="concepts_book_id_fkey"
        ),
        ForeignKeyConstraint(
            ["chapter_id"],
            ["chapters.id"],
            ondelete="SET NULL",
            name="concepts_chapter_id_fkey",
        ),
        PrimaryKeyConstraint("id", name="concepts_pkey"),
        UniqueConstraint(
            "book_id", "name", "graph_version", name="uq_concept_book_name_version"
        ),
...
class ConceptEdge(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            ["books.id"],
            ondelete="CASCADE",
            name="concept_edges_book_id_fkey",
        ),
        ForeignKeyConstraint(
            ["from_concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="concept_edges_from_concept_id_fkey",
        ),
        ForeignKeyConstraint(
            ["to_concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="concept_edges_to_concept_id_fkey",
        ),
    ...
    graph_version = Column(Integer, nullable=False, default=1)
    from_concept_id = Column(
        UUID(as_uuid=True),
        ForeignKey("concepts.id", ondelete="CASCADE"),
        nullable=False,
    )
    to_concept_id = Column(
        UUID(as_uuid=True),
        ForeignKey("concepts.id", ondelete="CASCADE"),
        nullable=False,
    )
    edge_type = Column(
        ENUM("PREREQUISITE", "RELATED", name="edge_type", create_type=False),
        nullable=False,
...
class RawConcept(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["graph_versions.id"],
            ondelete="CASCADE",
            name="raw_concepts_graph_version_id_fkey",
        ),
    ...
    )  # Not FK to avoid NoReferencedTableError since SourceChunk model is missing
    name = Column(Text, nullable=False)
    summary = Column(Text, nullable=False)
    difficulty_level = Column(Integer, nullable=False)
...
class RelationshipCandidate(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["graph_versions.id"],
            ondelete="CASCADE",
            name="relationship_candidates_graph_version_id_fkey",
        ),
        ForeignKeyConstraint(
            ["source_concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="relationship_candidates_source_concept_id_fkey",
        ),
        ForeignKeyConstraint(
            ["target_concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="relationship_candidates_target_concept_id_fkey",
        ),
...
class EvaluatedPair(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["graph_versions.id"],
            ondelete="CASCADE",
            name="evaluated_pairs_graph_version_id_fkey",
        ),
        ForeignKeyConstraint(
            ["source_concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="evaluated_pairs_source_concept_id_fkey",
        ),
        ForeignKeyConstraint(
            ["target_concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="evaluated_pairs_target_concept_id_fkey",
        ),
        ...
        UniqueConstraint(
            ...
            "source_concept_id",
            "target_concept_id",
            name="uq_evaluated_pairs_version_src_tgt",
        ),
...
class SourceChunk(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "page_start IS NULL AND page_end IS NULL OR page_start IS NOT NULL AND page_end IS NOT NULL AND page_end >= page_start",
            name="chk_page_numbers",
        ),
        CheckConstraint(
            "token_count IS NULL OR token_count > 0", name="chk_token_count"
        ),
        ForeignKeyConstraint(
            ["book_id"],
            ["books.id"],
            ondelete="CASCADE",
            name="source_chunks_book_id_fkey",
        ),
        ForeignKeyConstraint(
            ["book_upload_id"],
            ["book_uploads.id"],
            ondelete="CASCADE",
            name="source_chunks_book_upload_id_fkey",
        ),
        ForeignKeyConstraint(
            ["chapter_id"],
            ["chapters.id"],
            ondelete="SET NULL",
            name="source_chunks_chapter_id_fkey",
        ),
...
class ConceptChunk(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "relevance_score IS NULL OR relevance_score >= 0::numeric AND relevance_score <= 1::numeric",
            name="chk_relevance_score",
        ),
        ForeignKeyConstraint(
            ["chunk_id"],
            ["source_chunks.id"],
            ondelete="CASCADE",
            name="concept_chunks_chunk_id_fkey",
        ),
        ForeignKeyConstraint(
            ["concept_id"],
            ["concepts.id"],
            ondelete="CASCADE",
            name="concept_chunks_concept_id_fkey",
        ),
