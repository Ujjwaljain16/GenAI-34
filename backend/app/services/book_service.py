...
from app.models.book import Book, GraphVersion, UserBook
from app.repositories.book_repo import BookRepository
from app.schemas.book import BookDTO, JobStatusDTO, BookStatusDTO
...
class BookService:
    ...
    async def create_book(
        self, user_id: str, title: str, author: str, description: str, is_public: bool
    ) -> BookDTO:
        book = Book(
            owner_id=user_id,
            title=title,
    ...
    async def upload_book_file(
        self, book_id: str, user_id: str, file: UploadFile
    ) -> JobStatusDTO:
    ...
    async def get_book_graph(self, book_id: str):
        ...
        concept_rows = await self.book_repo.session.execute(
            text("""
        ...
        edge_rows = await self.book_repo.session.execute(
            text("""
...
async def mock_process_book_job(job_id: str, book_id: str, user_id: str, session):
    ...
    version = GraphVersion(
        book_id=book_id, version=job.graph_version, build_job_id=job.id
    )
    version = await repo.create_version(version)

    # Tie to user
    user_book = UserBook(
        user_id=user_id, book_id=book_id, pinned_graph_version_id=version.id
    )
