...
class RevisionService:
    ...
    async def get_due(self, user_id: str, book_id: str) -> DueListDTO:
        ...
        return DueListDTO(
            bookId=book_id, count=len(due),
            due=[DueItemDTO(conceptId=d["concept_id"], title=d["title"],
                            nextDue=d["next_due"], retrievability=d["retrievability"])
                 for d in due],
        )

    async def review(self, user_id: str, book_id: str, concept_id: str, grade: int) -> ReviewResultDTO:
        if not await self.graph.is_enrolled(user_id, book_id):
            raise HTTPException(status_code=404, detail="Book not found in your library.")
        ...
        return ReviewResultDTO(
            conceptId=concept_id, grade=grade, mastery=r["mastery"],
            intervalDays=r["interval_days"], stability=r["stability"], difficulty=r["difficulty"],
        )
