...
class GraphRepair:
    ...
    @staticmethod
    def repair_cycles(
        cycles: List[List[str]], edges: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for cycle in cycles:
            ...
            for i in range(len(cycle) - 1):
                ...
                for e in edges:
                    if (
                        e["source_concept_id"] == src
                        and e["target_concept_id"] == tgt
            ...
            if cycle[0] != cycle[-1]:
                for e in edges:
                    if (
                        e["source_concept_id"] == cycle[-1]
                        and e["target_concept_id"] == cycle[0]
                        and e["relationship_type"] == "PREREQUISITE"
                    ):
                        cycle_edges.append(e)
            ...
            if cycle_edges:
                ...
                lowest_confidence_edge = min(
                    cycle_edges, key=lambda x: x.get("confidence", 1.0)
                )
