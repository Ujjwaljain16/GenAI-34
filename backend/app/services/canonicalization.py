...
class CanonicalizationEngine:
    ...
    def find_duplicates(self, new_concept: Dict[str, Any], existing_concepts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def group_candidates(self, candidates: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        """
        ...
        while unassigned:
            ...
            while i < len(unassigned):
                ...
                score = fuzz.token_sort_ratio(current.get("name", "").lower(), candidate.get("name", "").lower())
                if score >= self.similarity_threshold:
